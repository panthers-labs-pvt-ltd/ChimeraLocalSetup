...
VERSION_GLAREDB=$(getVersionToInstall "glaredb")
...
if [ $miniconda_installed = true ]; then
  ...
  if [[ -d ~/miniconda3/lib/python3.12/site-packages/glaredb ]]; then
    version_string=$( pip show glaredb | grep "Version" )
    if [[ $version_string == *"${VERSION_GLAREDB}"* ]]; then
      echo "GlareDB version ${VERSION_GLAREDB} is already installed"
      installed=true
    fi
  fi
fi
...
if [ $installed = false ]; then
  source activate base
  pip install glaredb
  echo -e "\nGlareDB is now installed in miniconda's base environment."
fi
